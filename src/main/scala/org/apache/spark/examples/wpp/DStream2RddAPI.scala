...
object DStream2RddAPI {
  def main(args: Array[String]) {
    //1、创建sparkConf
    val sparkConf: SparkConf = new SparkConf().setAppName("DStream2RddAPI").setMaster("local[2]")
    //2、创建sparkContext
...
//    val socketDStream: ReceiverInputDStream[String] = ssc.socketTextStream("bigdata.ibeifeng.com",9999)
    val socketDStream: ReceiverInputDStream[String] = ssc.socketTextStream("10.3.1.9",9999)

    //dstream 当中有一些api是没有的（例如：sortbyKey等）
    //将DStream转换成RDD进行操作
    val resultDStream: DStream[((String, String), Int)] = socketDStream
      .transform((rdd,timestamp) =>{
